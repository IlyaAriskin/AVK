...
def send_next(user_id):
    ...
    keyboard.add_button("blacklist", VkKeyboardColor.SECONDARY)
    keyboard.add_button("Завершить", VkKeyboardColor.NEGATIVE)
    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW and event.to_me:
            ...
            if msg and flag == "":
                ...
                vkinder.add_to_favourite(user.user_id, user.first_name, user.last_name)
                keyboard = VkKeyboard(one_time=False, inline=True)
                keyboard.add_button("ИЗБРАННОЕ", VkKeyboardColor.PRIMARY)
                keyboard.add_button("Дальше", VkKeyboardColor.SECONDARY)
                keyboard.add_line()
                ...
                for favorite in favorites:
                    ...
                    full_name = f"{favorite.first_name} {favorite.last_name}"
                    page_link = f"https://vk.com/id{favorite.user_id}"
                ...
                vkinder.add_to_blacklist(user.user_id, user.first_name, user.last_name)
                keyboard = VkKeyboard(one_time=False, inline=True)
                keyboard.add_button("Дальше", VkKeyboardColor.SECONDARY)
                keyboard.add_button("Завершить", VkKeyboardColor.NEGATIVE)
                send_msg(
                    client_id,
                    f"Пользователь {full_name} {page_link} добавлен в черный список!",
                    keyboard,
                )
            elif msg == "blacklist":
                blacklisted = vkinder.get_blacklist()
                send_msg(client_id, f"У Вас в черном списке {len(blacklisted)} человек:")
                for blacklists in blacklisted:
                    user = vkinder.user_search(user_id=blacklists.user_id)
                    full_name = f"{blacklists.first_name} {blacklists.last_name}"
                    page_link = f"https://vk.com/id{blacklists.user_id}"
                    send_msg(client_id, f"{full_name} {page_link}")
                keyboard = VkKeyboard(one_time=False, inline=True)
                keyboard.add_button("Дальше", VkKeyboardColor.SECONDARY)
                keyboard.add_line()
